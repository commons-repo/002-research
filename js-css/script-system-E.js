...
let xData = [];
let yData = [];
let spreadsheetInstances = [null, null, null, null]; // 4 sheets
...
function openSpreadsheet() {
    ...
    // Initialize all sheets if not already created
    if (!spreadsheetInstances[0]) {
        for (let i = 0; i < 4; i++) {
            initializeSheet(i);
        }
    }
...
function switchSheet(sheetIndex) {
    ...
    // Update sheet container visibility
...
function exportSpreadsheetCSV() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `sheet${activeSheetIndex + 1}_data.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
...
function importSpreadsheetCSV(event) {
    ...
    reader.onload = function(e) {
        ...
        const activeSheet = spreadsheetInstances[activeSheetIndex];
        if (activeSheet) {
            activeSheet.setData(data.slice(0, 3000));
            alert(`CSV imported successfully into Sheet ${activeSheetIndex + 1}! Loaded ${Math.min(lines.length, 3000)} rows.`);
        }
    };
    reader.readAsText(file);
...
function clearSpreadsheet() {
    const activeSheet = spreadsheetInstances[activeSheetIndex];
    if (activeSheet && confirm(`Clear all data in Sheet ${activeSheetIndex + 1}?`)) {
        activeSheet.setData(Array(3000).fill(null).map(() => Array(26).fill('')));
    }
}
